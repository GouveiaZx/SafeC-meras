...
class StreamingService {
  constructor() {
    ...
    this.preferredServer = 'zlm'; // PRODUÇÃO: Sempre usar ZLMediaKit // 'srs', 'zlm' ou 'simulation' - updated
  ...
  async init() {
    ...
    try {
      ...
      // PRODUÇÃO: Configurado para usar apenas ZLMediaKit - Mocks e simulações removidos
      this.preferredServer = 'zlm';
      this.usesFallback = false;
      this.fallbackService = null;
      
      logger.info('MODO PRODUÇÃO: Usando apenas ZLMediaKit - Simulações desabilitadas');
      
      // Verificar conectividade apenas do ZLMediaKit
      const connectivityTests = await this.testConnectivity();
      const zlmTest = connectivityTests.find(test => test.server === 'zlm');
      
      if (!zlmTest || zlmTest.status !== 'online') {
        throw new Error('ZLMediaKit não está disponível. Verifique se o servidor está rodando na porta 9902.');
      }
      
      logger.info('ZLMediaKit conectado com sucesso');
  ...
  async startStream(camera, options = {}) {
    try {
      ...
      // USAR APENAS ZLMediaKit - SEM FALLBACKS OU SIMULAÇÕES
      let streamConfig;
      if (this.preferredServer === 'zlm') {
        streamConfig = await this.startZLMStream(camera, { quality, format, audio, streamId, userToken });
      } else {
        throw new Error('Apenas ZLMediaKit é suportado. Simulações e fallbacks foram desabilitados.');
      }
  ...
  async stopStream(streamId, userId) {
    try {
      ...
      // PARAR STREAM APENAS NO ZLMediaKit
      if (stream.server === 'zlm') {
        await this.stopZLMStream(stream);
      } else {
        logger.warn(`Stream ${streamId} não é do ZLMediaKit, ignorando parada`);
      }
