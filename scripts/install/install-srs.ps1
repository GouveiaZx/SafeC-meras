...
param(
    [string]$InstallPath = (Join-Path $env:ProgramData 'srs'),
    [string]$RtmpPort = "1935",
...
Write-Host "- Diretorio de instalacao: $InstallPath" -ForegroundColor White
Write-Host "- Porta RTMP: $RtmpPort" -ForegroundColor White
Write-Host "- Porta HTTP: $HttpPort" -ForegroundColor White
Write-Host "- Porta API: $ApiPort" -ForegroundColor White
...
$configSource = Join-Path $ProjectPath 'docker\srs\srs.conf'
...
if (Test-Path $srsExe) {
    ...
    Write-Host "- RTMP: rtmp://localhost:$RtmpPort/" -ForegroundColor Cyan
Write-Host "- HTTP-FLV: http://localhost:$HttpPort/" -ForegroundColor Cyan
Write-Host "- API: http://localhost:$ApiPort/api/v1/" -ForegroundColor Cyan
    ...
    Write-Host "Extraia para: $srsDir" -ForegroundColor Yellow
}
"@
...
Write-Host "3. Extraia para: $srsDir" -ForegroundColor White
Write-Host ""
Write-Host "URLs de acesso:" -ForegroundColor Yellow
Write-Host "- RTMP: rtmp://localhost:$RtmpPort/" -ForegroundColor White
Write-Host "- HTTP-FLV: http://localhost:$HttpPort/" -ForegroundColor White
Write-Host "- API: http://localhost:$ApiPort/api/v1/" -ForegroundColor White
Write-Host "- HLS: http://localhost:$HttpPort/live/stream.m3u8" -ForegroundColor White